...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        correct = torch.logical_and(pred>=target*args.K//args.num_class , pred<(target+1)*args.K//args.num_class)
        # correct = pred==target*args.K//args.num_class
        # correct_1 = pred.eq(target.reshape(1, -1))
        # if args.rank==0:
        #     print(correct==correct_1)
            
        res = []
        for k in topk:
            correct_k = (correct[:k].float().sum(dim=0)>0).float().sum(0,keepdim=True)
...
def train(loader, model, optimizer, epoch, scaler):
    ...
    for it, (img1,img2, label) in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.use_scaler):
            ...
            if args.simloss:
                loss1 = (mask_CE(py1,label,args.K,args.num_class) + mask_CE(py2,label,args.K,args.num_class))/2
                py1 = F.normalize(py1*label_mask,dim=1)
                py2 = F.normalize(py2*label_mask,dim=1)
                emb1 = F.normalize(emb1*label_mask,dim=1)
                emb2 = F.normalize(emb2*label_mask,dim=1)
                loss2 = -((py1*emb2).sum(1).mean()+(py2*emb1).sum(1).mean())/2
                loss = args.alpha*loss2+loss1
            else:
                
                with torch.no_grad():
                    
                    emb1 = F.normalize(F.softmax(emb1/args.temperature,dim=1)*label_mask,p=1,dim=1)
                    emb2 = F.normalize(F.softmax(emb2/args.temperature,dim=1)*label_mask,p=1,dim=1)
                    # _,label1 = torch.max(emb1,dim=1)
                    # _,label2 = torch.max(emb2,dim=1)
                    

                py1 = F.log_softmax(py1/args.temperature, dim=1)
                py2 = F.log_softmax(py2/args.temperature, dim=1)
                # loss = -(py1*emb1).sum(dim=1).mean() 
                # loss = F.cross_entropy(py1/args.temperature, label)
                # loss = (F.cross_entropy(py1/args.temperature,label2)+F.cross_entropy(py2/args.temperature,label1))/2 # hard
                # loss = F.cross_entropy(py1/args.temperature,label1) # hard
...
def mask_CE(logits,labels,K,classes):
    # logits_max, _ = torch.max(logits, dim=1, keepdim=True)
    # logits = logits - logits_max.detach()
    bs = logits.size(0)
    prob = F.softmax(logits,dim=1)
    loss = -(torch.log(prob.view(bs,classes,K//classes).sum(2))*make_one_hot(labels.view(-1,1),classes,1)).sum(1).mean()
    return loss
