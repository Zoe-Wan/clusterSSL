...
def train(loader, model, optimizer, epoch, scaler):
    ...
    for it, (img1, label) in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if args.simloss:
                ...
                with torch.no_grad():
                    label_mask = make_one_hot(label.view(-1, 1),args.num_class,args.K//args.num_class)
                    emb1 = F.normalize(F.softmax(emb1/args.temperature,dim=1)*label_mask,p=1)
                    # emb2 = F.normalize(F.softmax(emb2/args.temperature,dim=1)*label_mask,p=1)

                    # logger.info(emb1.sum(dim=1))
